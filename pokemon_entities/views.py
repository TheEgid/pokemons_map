...
def make_image_url(pokemon):
    return os.path.abspath(str(pokemon.img_url.url[1:]))
...
def show_all_pokemons(request):
    ...
    for pokemon_entity in PokemonEntity.objects.all():
        img_url = make_image_url(pokemon_entity.pokemon)
        add_pokemon(
            folium_map,
            pokemon_entity.lat,
            pokemon_entity.lon,
...
def show_pokemon(request, pokemon_id):
    ...
    for pokemon_entity in PokemonEntity.objects.filter(pokemon_id=requested_pokemon.id):
        img_url = make_image_url(requested_pokemon)
